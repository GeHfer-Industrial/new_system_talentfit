...
export const EDUCATION_LEVELS = ['ENSINO_MEDIO', 'TECNICO', 'SUPERIOR', 'POS_GRADUACAO', 'MESTRADO', 'DOUTORADO'];
export const EDUCATION_STATUSES = ['EM_ANDAMENTO', 'CONCLUIDO', 'TRANCADO'];
export const LANGUAGE_LEVELS = ['BASICO', 'INTERMEDIARIO', 'AVANCADO', 'FLUENTE'];
...
export function classificationFallback(engine: string): ClassificationResult {
  return {
    jobId: null,
    score: 0,
...
export function sanitizeExperiences(value: unknown): CandidateExperience[] {
...
export function sanitizeEducations(value: unknown): CandidateEducation[] {
  if (!Array.isArray(value)) return [];
  return (value as any[])
...
export function sanitizeLanguages(value: unknown): CandidateLanguage[] {
  if (!Array.isArray(value)) return [];
  return (value as any[])
...
export function buildClassificationPrompt(
  ...
  return `Você é um sistema especializado em triagem de currículos para RH. Analise o currículo abaixo.
...
export function parseClassificationResponse(text: string, engine: string): ClassificationResult {
  const jsonMatch = text.match(/\{[\s\S]*\}/);
  if (!jsonMatch) throw new Error(`No JSON in ${engine} response`);
